-- CRITICAL FIX: Make check_customer_duplicate TENANT-AWARE
-- Current bug: Checks duplicates across ALL tenants
-- Fix: Add tenant_id and is_live parameters

-- Drop old version
DROP FUNCTION IF EXISTS check_customer_duplicate(TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS check_customer_duplicate(VARCHAR, VARCHAR, VARCHAR) CASCADE;

-- Create TENANT-AWARE version
...
DECLARE
    v_exists BOOLEAN;
BEGIN
    -- Check by PAN if provided (PLAIN TEXT comparison) - TENANT-AWARE
...
COMMENT ON FUNCTION check_customer_duplicate IS 'TENANT-AWARE: Check for duplicate customers using PAN, email, or mobile within same tenant and environment';

-- Now recreate process_single_customer_record to use the new signature
...
BEGIN
    ...
    RAISE NOTICE '[DEBUG] Processing staging_id: %, tenant: %, mapped_data keys: %',
        p_staging_id,
        v_staging.tenant_id,
        (SELECT array_agg(key) FROM jsonb_object_keys(v_mapped_data) AS key);

    BEGIN
        -- Check for duplicates - NOW TENANT-AWARE!
        ...
        IF v_is_duplicate THEN
            ...
            SET processing_status = 'duplicate',
                warnings = array_append(warnings, 'Customer already exists in this tenant'),
...
COMMENT ON FUNCTION process_single_customer_record IS 'Process single customer record from staging - TENANT-AWARE duplicate checking - with family field support';

-- Verification
SELECT 'TENANT-AWARE FUNCTIONS DEPLOYED!' as status;

SELECT
    p.proname as function_name,
    pg_get_function_identity_arguments(p.oid) as arguments
FROM pg_proc p
WHERE p.proname IN ('check_customer_duplicate', 'process_single_customer_record')
ORDER BY p.proname;
