-- Fix for Family Field Import Issue
-- This script updates the process_single_customer_record function with debug logging
-- to help diagnose why family fields are becoming NULL

-- Drop and recreate the function with debug logging
...
COMMENT ON FUNCTION process_single_customer_record IS 'Process single customer record from staging - uses plain text PAN/IWELL - with debug logging for family fields';

-- Test query to check recent imports
SELECT
    'After fixing the function, run an import and check the PostgreSQL logs for [DEBUG] messages' AS instructions,
    'You can view logs with: docker logs kewalinvest_db 2>&1 | grep -i debug' AS log_command;
...
WHERE s.import_type = 'CustomerData'
  ...
  AND (s.mapped_data->>'family_head_name' IS NOT NULL OR s.mapped_data->>'family_head_iwell_code' IS NOT NULL)
