-- Fix: Remove ALL versions of check_customer_duplicate and recreate with correct signature
-- This handles the "function name is not unique" error

-- Step 1: Drop ALL versions of the function (using CASCADE to handle dependencies)
DROP FUNCTION IF EXISTS check_customer_duplicate CASCADE;

-- Step 2: Create the correct version with TEXT parameters (not VARCHAR)
...
-- Step 3: Recreate process_single_customer_record (it was dropped by CASCADE)
...
-- Verification
SELECT 'Functions fixed and deployed successfully!' as status;

-- Show the final function signature
SELECT
...
WHERE n.nspname = 'public'
  AND p.proname IN ('check_customer_duplicate', 'process_single_customer_record')
ORDER BY p.proname;
