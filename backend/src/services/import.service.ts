...
async processSchemeImport(
  ...
  try {
    ...
    // Post-import: Ensure scheme categories exist in t_scheme_masters
    // Insert the 42 AMFI scheme categories if they don't exist
    await client.query(`
    ...
    // Try both t_scheme_staging_data and t_import_staging_data tables

    // From t_scheme_staging_data
    await client.query(`
    ...
    // Also try from t_import_staging_data (in case scheme import uses general staging)
    await client.query(`
      UPDATE t_scheme_details sd
      SET scheme_category_id = sm.id
      ...
      JOIN t_scheme_masters sm ON LOWER(TRIM(sm.name)) = LOWER(TRIM(isd.mapped_data->>'scheme_category'))
        AND sm.master_type = 'scheme_category'
        AND sm.is_active = true
      WHERE isd.session_id = $1
        AND sd.tenant_id = $2
        AND sd.is_live = $3
        AND sd.scheme_code = isd.mapped_data->>'scheme_code'
    ...
    // Also try from t_import_staging_data
    await client.query(`
      UPDATE t_scheme_details sd
      SET scheme_type_id = sm.id
      FROM t_import_staging_data isd
