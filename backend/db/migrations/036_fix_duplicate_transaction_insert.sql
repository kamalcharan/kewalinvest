-- Migration: 036_fix_duplicate_transaction_insert.sql
-- Description: Fix duplicate detection to INSERT transaction with is_potential_duplicate=true
--              instead of skipping the insert entirely. This ensures duplicates appear
--              in the Transactions UI with proper tagging.
-- Date: 2026-03-31
...
DECLARE
    ...
    -- New variable for duplicate detection
...
BEGIN
    ...
    FOR v_staging_record IN (
        ...
        BEGIN
            ...
            -- DUPLICATE CHECK: set flag instead of skipping
            IF EXISTS (
            ...
            -- INSERT TRANSACTION (with duplicate tagging if detected)
            INSERT INTO t_transaction_table (
                ...
                v_asset_type_code,
                v_is_duplicate,
                CASE WHEN v_is_duplicate THEN
                    'Import duplicate: matching customer + scheme + date + amount (session ' || p_session_id || ')'
                ELSE NULL END,
                NOT v_is_duplicate,
                NOW(),
                NOW()
            ...
            -- Skip investment plan creation for duplicates
            -- ============================================================================
            IF v_asset_type_id IS NOT NULL AND NOT v_is_duplicate THEN
            ...
            -- Update staging status: for non-duplicates mark as success
...
COMMENT ON FUNCTION process_transaction_import_session IS 'Process transaction imports with customer lookup, PAN fallback, orphan tracking, duplicate tagging, and auto MF assignment with alerts';
